...
A practice phase — the whole generate/engrave/seal sequence run on a scrap plate with worthless test entropy — should precede anyone's first live mint. Learner's permit, then the real thing; see `type-s-workshop.md` for how that training actually runs, and `type-s-ceremony.md` for the live-mint checklist itself. See [[anyone-can-make-this-money]] for why this matters beyond due diligence: mastery, not just correctness, is what eventually lets the maker's doubt disappear entirely.
