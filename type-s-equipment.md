...
| **Raw materials** — steel plate blank, casing feedstock | What actually becomes the coin | ⚠️ | **Always brought by the minter. Never supplied by the workshop.** This is a hard rule, not a convenience — see Ownership model below. |

---

## Ownership and access model

**Hard rule: the workshop supplies tools, never materials.** Anything that becomes part of a finished coin — plate, casing feedstock, anything else consumed into the object itself — is always brought by the minter, never provided by the workshop. This isn't a resourcing detail; it's the line that keeps the workshop a tool shed and not a supplier, consistent with the growth model in `type-s-workshop.md` and the reasoning that keeps Type S outside money-transmission territory in the first place.

Four different categories, not one:
...
- **Individually owned, brought fresh each time**: raw materials (plate, casing feedstock). Never workshop-supplied, per the hard rule above.
